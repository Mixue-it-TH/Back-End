...
public class TaskService {
    ...
    private Integer MAX_FILESIZE = 10;
...
//    }
    ...
    public TaskDTO updateTask(String boardId, Integer taskId, TaskAddEditDTO editedTask, HttpServletRequest request) {
        ...
        List<FileDTO> arrayAdd = editedTask.getArrayAdd();
        if (arrayAdd != null && !arrayAdd.isEmpty()) {
            // หากไม่พบไฟล์ชื่อเดียวกัน ให้เพิ่มไฟล์ใหม่
            Map<String, Object> addResult = addFiles(boardId, taskId, arrayAdd, request);
...
//    }
    ...
    public List<TaskBoardDTO> getTaskofBoard(String boardId, HttpServletRequest request) {
        ...
        for (TaskBoardDTO taskDTO : taskBoardDTOList) {
            List<FileDTO> fileDTOs = taskBoard.stream()
                    ...
                    .flatMap(task -> task.getFiles().stream().map(file -> new FileDTO(file.getId(),file.getName(), file.getUrl())))
    ...
    public TaskSelectedDTO getTaskById(String boardId, int id, HttpServletRequest request) throws ItemNotFoundException {
        ...
        List<FileDTO> fileDTOs = task.getFiles().stream()
                .map(file -> new FileDTO(file.getId(), file.getName(), file.getUrl()))
                .collect(Collectors.toList());
    ...
    @Transactional
    public Map<String, Object> addFiles(String boardId, Integer taskId, List<FileDTO> fileDTOList, HttpServletRequest request) {
        ...
        int remainingSlots = MAX_FILESIZE - existingFiles.size();

        // If no space, throw an error
        if (remainingSlots <= 0) {
        ...
        for (int i = 0; i < fileDTOList.size(); i++) {
            ...
            if (i < remainingSlots) {
                ...
                boolean fileExists = fileRepository.findFileByTasks_IdAndName(taskId, dto.getName()) != null;

                if (fileExists) {
                    throw new ConflictException("This file " + dto.getName() + " already exists for the current task! Please Delete  attachment and add again");
                }

                // Create a new file
                File newFile = new File();
                newFile.setId(NanoId.generate(10));
        ...
        List<FileDTO> addedFileDTOs = filesToAdd.stream()
                .map(f -> new FileDTO(f.getId(), f.getName(), f.getUrl()))
                .collect(Collectors.toList());
    ...
    @Transactional
    public List<FileDTO> deleteFiles(Integer taskId, List<String> fileIds, HttpServletRequest request) {
        List<FileDTO> deletedFiles = new ArrayList<>();
        ...
        for (String fileId : fileIds) {
            ...
            deletedFiles.add(new FileDTO(fileId, fileResultDelete.getName(), fileResultDelete.getUrl()));  // คุณสามารถตั้งค่าข้อมูลไฟล์ที่ต้องการส่งกลับได้
        }
